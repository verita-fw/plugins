...
class Cmd():
    ...
    def getBootstrapOutput(self, resObj):
        """ Returns results gathered from Primary Server Load Balancer as jquery bootstrap output """
        args = {
            "rows": resObj,
    ...
    def getJsonOutput(self, resObj):
        """Returns results gathered from Primary Server Load Balancer as json"""
        return json.dumps(resObj)

    def getResult(self, res):
        """Returns raw results gathered from Primary Server Load Balancer"""
        if res == '\n':
            res = None
    ...
    def getResultObj(self, res):
        """Returns refined output from Primary Server Load Balancer, packed inside a Python obj i.e. a dict()"""
        return res
...
class newSslCrt(Cmd):
    """" Create an empty slot for the certificate in Primary Server Load Balancer memory """
...
class updateSslCrt(Cmd):
    """" Begin a transaction to upload the certificate into a slot in Primary Server Load Balancer memory """
    cmdTxt = "set ssl cert %(certfile)s <<\n%(payload)s\r\n"
    req_args = ['certfile', 'payload']
    helpTxt = "Replace a certificate file."

class delSslCrt(Cmd):
    """" Begin a transaction to remove the certificate from a slot in Primary Server Load Balancer memory """
    cmdTxt = "del ssl cert %(certfile)s\r\n"
    req_args = ['certfile']
    helpTxt = "Delete delete an unused certificate file."

class commitSslCrt(Cmd):
    """ Commit the transaction so Primary Server Load Balancer detects the change. """
...
class showInfo(Cmd):
    """Show info Primary Server Load Balancer command"""
    cmdTxt = "show info\r\n"
    helpTxt = "Show info on Primary Server Load Balancer instance."
